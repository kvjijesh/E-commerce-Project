...
const editProductLoad= async (req,res)=>{
    try {
        ...
        if(prodData){
            ...
            res.render('adminViews/edit',{products:prodData})
        }
        else{
...
const userproductload = async (req, res) => {
    ...
    const categories= await Category.find()
      ...
      const productData = await Product.find(filter).sort(sortOption)
        .populate("category");

      if (req.session.userData) {
        const userData = req.session.user;
        res.render("userViews/producthome", { products: productData, userData ,categories});
      } else {
        res.render("userViews/producthome", { products: productData,categories });
      }
    } catch (error) {
      res.status(500).json({
        message: error.message,
...
const productDetailLoad= async(req,res)=>{
    try {
        ...
        if(prodData){
            // console.log(userData.name);
            res.render('userViews/productDetail',{products:prodData,userData})
        }
        else{
...
const menProductLoad= async(req,res)=>{
    ...
    const categoryId=req.query.id
    const productmen= await Product.find({ category: categoryId,is_blocked:false })
